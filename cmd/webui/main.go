...
type InterLinkConfig struct {
	...
	Plugin string `yaml:"plugin,omitempty"` // "docker" or "slurm"
}
...
type SSHTunnelConfig struct {
	...
	// Base64-encoded keys for Kubernetes secrets
	PrivateKeyB64 string `yaml:"-"`
	PublicKeyB64  string `yaml:"-"`
}
...
func initTemplates() error {
	...
	// Load Helm values template from embedded filesystem
	helmContent, err := templateFS.ReadFile("templates/helm-values.yaml")
	if err != nil {
		return fmt.Errorf("failed to read helm template file: %w", err)
	}
	helmTemplate, err = template.New("helm").Parse(string(helmContent))
	if err != nil {
		return fmt.Errorf("failed to parse helm template: %w", err)
	}

	// Load installation script template from embedded filesystem
	scriptContent, err := templateFS.ReadFile("templates/install-script.sh")
	if err != nil {
		return fmt.Errorf("failed to read script template file: %w", err)
	}
	scriptTemplate, err = template.New("script").Parse(string(scriptContent))
	if err != nil {
...
func configureHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		configData.Plugin = r.FormValue("plugin")
		...
		if configData.DeploymentMode == deploymentTunneled {
			...
			// Base64 encode the keys for Kubernetes secrets
			configData.SSHTunnel.PrivateKeyB64 = base64.StdEncoding.EncodeToString([]byte(sshKeys.PrivateKey))
			configData.SSHTunnel.PublicKeyB64 = base64.StdEncoding.EncodeToString([]byte(sshKeys.PublicKeyOpenSSH))
...
<body>
    ...
    <form hx-post="/configure" hx-trigger="submit" hx-target="body">
        ...
        <div class="section">
            ...
            <div class="form-group">
                <label for="plugin">Plugin Type:</label>
                <select id="plugin" name="plugin" onchange="togglePluginSections()">
                    <option value="docker" {{if eq .ConfigData.Plugin "docker"}}selected{{end}}>Docker</option>
                    <option value="slurm" {{if eq .ConfigData.Plugin "slurm"}}selected{{end}}>SLURM</option>
                </select>
                <div class="field-help">
                    Choose the execution environment plugin for running containers.
                    <br><strong>Docker:</strong> Container execution using Docker runtime (general purpose)
                    <br><strong>SLURM:</strong> HPC job submission to SLURM workload manager (for batch processing, scientific computing)
                </div>
            </div>
            ...
            <div id="docker-section" style="display: none;">
                <h4>🐳 Docker Plugin Configuration</h4>
                <div style="background: linear-gradient(45deg, #e3f2fd, #bbdefb); border: 1px solid #2196f3; border-radius: 8px; padding: 10px; margin: 10px 0;">
                    <p style="margin: 0; color: #0d47a1;"><strong>🐳 Docker Plugin Features:</strong></p>
                    <ul style="margin: 5px 0 0 20px; color: #0d47a1;">
                        <li>Direct container execution using Docker runtime</li>
                        <li>Support for standard Docker images and registries</li>
                        <li>Ideal for general-purpose container workloads</li>
                        <li>Simple setup and configuration</li>
                    </ul>
                </div>
                <p style="color: #666; font-size: 0.9rem; margin: 10px 0;">
                    The Docker plugin executes containers directly using the Docker runtime on the target system. 
                    This is the most common choice for general container workloads and development environments.
                </p>
            </div>
            
            <div id="slurm-section" style="display: none;">
                <h4>⚡ SLURM Plugin Configuration</h4>
                <div style="background: linear-gradient(45deg, #fff3cd, #ffeaa7); border: 1px solid #fd7e14; border-radius: 8px; padding: 10px; margin: 10px 0;">
                    <p style="margin: 0; color: #e07500;"><strong>⚡ SLURM Plugin Features:</strong></p>
                    <ul style="margin: 5px 0 0 20px; color: #e07500;">
                        <li>HPC job submission to SLURM workload manager</li>
                        <li>Support for batch processing and queuing</li>
                        <li>Ideal for scientific computing and AI/ML workloads</li>
                        <li>Resource allocation and job scheduling</li>
                        <li>Integration with Apptainer/Singularity containers</li>
                    </ul>
                </div>
                <p style="color: #666; font-size: 0.9rem; margin: 10px 0;">
                    The SLURM plugin submits containers as HPC jobs to a SLURM cluster. 
                    Perfect for scientific computing, AI/ML training, and batch processing workloads that need specialized resources.
                </p>
            </div>
        </div>
    ...
    <script>
        ...
        function togglePluginSections() {
            ...
            // Hide all plugin sections first
            if (dockerSection) dockerSection.style.display = 'none';
            if (slurmSection) slurmSection.style.display = 'none';
            
            // Show the selected plugin section
            if (plugin === 'docker' && dockerSection) {
                dockerSection.style.display = 'block';
            } else if (plugin === 'slurm' && slurmSection) {
                slurmSection.style.display = 'block';
            }
        }
